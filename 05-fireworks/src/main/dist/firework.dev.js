...
function () {
  function Fireworks(color, to) {
    ...
    this.color = new Three.Color(color); // 创建烟花发射的球点
    ...
    this.startGeometry.setAttribute("aStep", new Three.BufferAttribute(astepArray, 3)); // 设置着色器材质
    ...
    for (var i = 0; i < this.FireworksCount; i++) {
      ...
      positionFireworksArray[i * 3 + 2] = to.z; //   设置烟花所有粒子初始化大小

      scaleFireArray[i] = Math.random(); //   设置四周发射的角度
